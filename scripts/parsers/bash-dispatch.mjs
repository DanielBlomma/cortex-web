/**
 * Bash parser dispatcher.
 *
 * Today only the tree-sitter parser exists for Bash. The dispatcher
 ...
 * is maintained.
 *
 *   CORTEX_BASH_PARSER=tree-sitter → force tree-sitter (default)
 *   CORTEX_BASH_PARSER=regex       → currently rejected (no regex parser)
 *   unset / other                  → tree-sitter
 */

const choice = process.env.CORTEX_BASH_PARSER;
...
async function resolveParser() {
  ...
  resolvePromise = (async () => {
    if (choice === "regex") {
      throw new Error(
        "CORTEX_BASH_PARSER=regex is not supported: no regex Bash parser is bundled. " +
          "Unset the variable or set it to 'tree-sitter'."
      );
    }
    activeParser = await import("./bash-treesitter.mjs");
    return activeParser;
  })();
