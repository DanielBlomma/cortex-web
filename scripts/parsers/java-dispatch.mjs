 ...
 *
 * Today only the tree-sitter parser exists for Java. The dispatcher
 ...
 * is maintained.
 *
 *   CORTEX_JAVA_PARSER=tree-sitter → force tree-sitter (default)
 *   CORTEX_JAVA_PARSER=regex       → currently rejected (no regex parser)
 *   unset / other                  → tree-sitter
 */

const choice = process.env.CORTEX_JAVA_PARSER;
...
async function resolveParser() {
  ...
  resolvePromise = (async () => {
    if (choice === "regex") {
      throw new Error(
        "CORTEX_JAVA_PARSER=regex is not supported: no regex Java parser is bundled. " +
          "Unset the variable or set it to 'tree-sitter'."
      );
    }
    activeParser = await import("./java-treesitter.mjs");
    return activeParser;
  })();
