/**
 * Python parser dispatcher.
 *
 * Today only the tree-sitter parser exists for Python. The dispatcher
 ...
 * is maintained.
 *
 *   CORTEX_PYTHON_PARSER=tree-sitter → force tree-sitter (default)
 *   CORTEX_PYTHON_PARSER=regex       → currently rejected (no regex parser)
 *   unset / other                    → tree-sitter
 */

const choice = process.env.CORTEX_PYTHON_PARSER;
...
async function resolveParser() {
  ...
  resolvePromise = (async () => {
    if (choice === "regex") {
      throw new Error(
        "CORTEX_PYTHON_PARSER=regex is not supported: no regex Python parser is bundled. " +
          "Unset the variable or set it to 'tree-sitter'."
      );
    }
    activeParser = await import("./python-treesitter.mjs");
    return activeParser;
  })();
